...
service based on progress JSDO javascript ... object. jsdo is progress - connector to the mobile services of Progress,which produce metadata. link to it. we have an extended catalog file which describes the data on their custom format. we parse it and use the data - catalog describes what the service will return and what its contraints are   
